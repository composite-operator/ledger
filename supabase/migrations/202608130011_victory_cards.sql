...
-- One-time victory alerts for positive closed outcomes and deep-linked share cards.

begin;

alter table public.notification_preferences
  add column if not exists notify_wins boolean not null default true;
...
create or replace function public.notify_victory_after_setup_close()
returns trigger
language plpgsql
security definer
set search_path = ''
as $$
declare
  was_victory boolean := false;
  is_victory boolean := false;
begin
  is_victory := (
    new.status in ('CLOSED', 'RESOLVED')
    and coalesce(new.r_result, new.score, 0) > 0
    and coalesce(new.final_status::text, '') not in ('STOPPED', 'CANCELLED', 'EXPIRED')
  );

  if tg_op = 'UPDATE' then
    was_victory := (
      old.status in ('CLOSED', 'RESOLVED')
      and coalesce(old.r_result, old.score, 0) > 0
      and coalesce(old.final_status::text, '') not in ('STOPPED', 'CANCELLED', 'EXPIRED')
    );
  end if;

  if not is_victory or was_victory then
    return new;
  end if;
  ...
  where coalesce(preferences.notifications_muted, false) = false
    and coalesce(preferences.notify_wins, true) = true
...
drop trigger if exists setups_notify_victory_after_insert on public.setups;
create trigger setups_notify_victory_after_insert
after insert on public.setups
for each row execute function public.notify_victory_after_setup_close();

drop trigger if exists setups_notify_victory_after_update on public.setups;
create trigger setups_notify_victory_after_update
...
revoke execute on function public.notify_victory_after_setup_close()
